  ...
  callback = function()
    ...
    local on_attach = function(_, bufnr)
      ...
      map(
        "n",
        "mcl",
        "/\\(public\\|private\\|protected\\)\\( class \\| abstract class \\| interface \\)<cr><esc>",
        "Move to class"
      )
      map(
        "n",
      ...
      vim.env.LOG4J_CLASSPATH = mvn_path
      ...
      vim.env.JACKSON_CLASSPATH = mvn_path
          ...
          .. mvn_path
          .. "/com/fasterxml/jackson/datatype/jackson-datatype-jdk8/2.13.5/jackson-datatype-jdk8-2.13.5.jar:"
          .. mvn_path
        ...
        if extracted_text then
          ...
          if extracted_text then
            ...
            local command = string.format(
              [[execute 'term java -classpath $LOG4J_CLASSPATH:$JACKSON_CLASSPATH:$ROOT_CLASSPATH %s']],
              main_class
            )
            print("cmd: ", command)
            vim.cmd(command)
      ...
      settings = {
        java = {
          ...
          format = {
            enabled = true,
            settings = {
              url = vim.fn.expand("$HOME") .. "/.config/nvim/lang-servers/intellij-java-google-style.xml",
              profile = "GoogleStyle",
            },
          },
        },
